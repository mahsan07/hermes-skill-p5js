...
This package expresses **p5.js** as a typed, task-adaptive agent harness. Its concrete capability remains: Build generative art, shaders, interactive sketches, and 3D scenes with p5.js. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in p5.js |
| --- | --- | --- |
| Memory | `artifact_revision_history` | Retain sketch concept plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `iterative_composition_plan` | Transform p5.js canvas into the ordered, bounded stages defined by this skill. |
...
| Action | `brief_generate_render_inspect_export` | Advance the workflow through drawing and shader code while preserving stop conditions and user authority. |
